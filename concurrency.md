...
_happens-before_ - логическое ограничение на порядок выполнения инструкций программы. Если указывается, что запись в переменную и последующее ее чтение связаны через эту зависимость, то как бы при выполнении не переупорядочивались инструкции, в момент чтения все связанные с процессом записи результаты уже зафиксированы и видны
...
1.	Прямой порядок операций в одном потоке:
	- В одном потоке все операции выполняются последовательно. Если операция A происходит перед операцией B в одном потоке, то A happens-before B.
2.	Блокировка и разблокировка:
	- Если поток T1 освобождает блокировку (с помощью synchronized), а поток T2 захватывает ту же блокировку, то все операции, которые были выполнены до освобождения блокировки в потоке T1, будут видны потоку T2 после захвата этой блокировки. Это означает, что освобождение блокировки happens-before захвата блокировки другим потоком.
3.	Запись в volatile переменную:
	- Запись в переменную, помеченную как volatile, happens-before последующее чтение этой переменной. Это означает, что изменения, сделанные в переменной до записи, будут видны другим потокам при чтении этой переменной.
4.	Начало и завершение потока:
	- Если поток A вызывает метод Thread.start() для потока B, то все операции в потоке A, которые предшествуют вызову start(), будут видны в потоке B после его запуска.
	- Если поток A вызывает Thread.join() для потока B и он завершается, то все операции, выполненные в потоке B, будут видны в потоке A после выхода из join().
5.	Передача объекта через Thread.start() или ExecutorService:
	- Передача объектов через механизм потока или пул потоков также образует отношения happens-before.
