...
+ [Как можно получить записи с помощью `Spring Jdbc Template`?](#Как-можно-получить-записи-с-помощью-Sprin-Jdbc-Template)
...
+ [Что такое `Hibernate Validator` система?](#Что-такое-Hibernate-Validator-система)
...
В Spring Framework термин "advice" относится к одной из основных составляющих аспектно-ориентированного программирования (Aspect-Oriented Programming, AOP). Advice (совет) представляет собой модуль, содержащий код, который выполняется в определенный момент времени в рамках выполнения приложения.
...
## Что такой паттерн прокси в `Spring AOP`?
 - Паттерн прокси в Spring AOP используется для добавления дополнительной функциональности к существующему объекту без изменения его кода. Это осуществляется путем создания посредника (прокси), который оборачивает целевой объект и перехватывает вызовы к нему.

 - Когда вы используете Spring и применяете аспекты к объекту, Spring создает прокси-объект, который становится посредником между клиентами и целевым объектом. Когда клиент обращается к прокси, он может добавить свою логику до, после или вокруг вызовов методов целевого объекта.

 - Прокси-объекты в Spring AOP могут использоваться, например, для обеспечения управления транзакциями или ленивой инициализации объектов. Они позволяют добавлять новое поведение к существующим объектам без необходимости изменения их кода напрямую.
