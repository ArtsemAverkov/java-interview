...
+ [Что такой паттерн `View Resolver` и объясните его значение в `Spring MVC`?](#Что-такой-паттерн-View-Resolver)
...
+ [Что такое `ContextLoaderListener` и что она делает?](#Что-такое-ContextLoaderListener)
+ [В чем разница между аннотациями `@RequestParam` и `@PathVariable`?](#В-чем-разница-между-аннотациями-@RequestParam-и-@PathVariable)
+ [Что такая модель `Model` в Spring MVC?](#Что-такое-Model-в-Spring-MVC)
...
+ [Являются ли синглтон-бины `(singleton beans)` потокобезопасными?](#Являются-ли-singleton-beans-потокобезопасными)
...
+ [Как можно использовать источник данных `Tomcat JNDI` Tomcat JNDI DataSource в приложениях Spring?](#Как-можно-использовать-источник-данных-Tomcat-JNDI)
...
[к оглавлению](#spring-MVC-Interview-Questions)

#Какие-преимущества-фреймворка-Spring-MVC
...
#Что-такой-DispatcherServlet-в-Spring-MVC
...
#Что-такой-паттерн-View-Resolver
...
#Можно-ли-создать-контроллер-без-использования-аннотаций-@Controller-или-@RestController
...
#Какие-типы -Dependency-Injection
...
#Как-можно-обеспечить-потокобезопасность-в-бинах
...
#Как-инициализируется-dispatcher-servlet
...
#Как-загружается-root-application-context-в-Spring MVC
...
#Откуда-происходит-доступ-к-model-из-view
...
#Что-такие-interceptors-в-Spring
...
#Нужно-ли-находиться-на-classpath-файлу-spring-mvc.jar
...
#Как-выполняется -алидация-данных-формы-в-фреймворке-Spring-Web-MVC
...
Как получить объекты `ServletConfig` и `ServletContext` в Spring-бине?
...
#Как-в-Spring-MVC-поддерживаются-i18n
...
#Как-можно-использовать-источник-данных-Tomcat-JNDI
