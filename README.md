...
+ [Какие преимущества фреймворка `Spring MVC` по сравнению с другими MVC фреймворками?](sm.md#Какие-преимущества-фреймворка-Spring-MVC-по-сравнению-с-другими-MVC-фреймворками)
+ [Что такой `DispatcherServlet` в `Spring MVC` Можете ли вы объяснить архитектуру Spring MVC?](sm.md#Что-такой-DispatcherServlet-в-Spring-MVC-Можете-ли-вы-объяснить-архитектуру-Spring-MVC)
+ [Что такой паттерн `View Resolver` и объясните его значение в `Spring MVC`?](sm.md#Что-такой-паттерн-View-Resolver-и-объясните-его-значение-в-Spring-MVC)
+ [Для чего используется аннотация `Controller`?](sm.md#Для-чего-используется-аннотация-Controller)
+ [Можно ли создать контроллер без использования аннотаций `Controller` или `RestControllerv`?](sm.md#Можно-ли-создать-контроллер-без-использования-аннотаций-Controller-или-RestController)
+ [Что такое `ContextLoaderListener` и что она делает?](sm.md#Что-такое-ContextLoaderListener-и-что-она-делает)
...
+ [Что такое модель `Model` в Spring MVC?](sm.md#Что-такое-модель-Model-в-Spring-MVC)
...
+ [Какова важность файла `web.xml` в `Spring MVC`?](sm.md#Какова-важность-файла-web.xml-в-Spring-MVC)
+ [Какие типы внедрения зависимостей `Dependency Injection` существуют в Spring MVC?](sm.md#Какие-типы-внедрения-зависимостей-Dependency-Injection-существуют-в-Spring-MVC)
...
+ [Являются ли синглтон-бины `singleton beans` потокобезопасными?](sm.md#Являются-ли-синглтон-бины-singleton-beans-потокобезопасными)
+ [Как можно обеспечить потокобезопасность в beans?](sm.md#Как-можно-обеспечить-потокобезопасность-в-beans)
+ [Каково значение аннотации `Repository`?](sm.md#Каково-значение-аннотации-Repository)
+ [Как инициализируется диспетчер сервлетов `dispatcher servlet`?](sm.md#Как-инициализируется-диспетчер-сервлетов-dispatcher-servlet)
+ [Как загружается корневой контекст приложения `root application context` в Spring MVC?](sm.md#Как-загружается-корневой-контекст-приложения-root-application-context-в-Spring-MVC)
...
+ [Откуда происходит доступ к модели `model` из представления `view`?](sm.md#Откуда-происходит-доступ-к-модели-model-из-представления-view)
+ [Зачем нам нужен `BindingResult`?](sm.md#Зачем-нам-нужен-BindingResult)
+ [Что такие интерцепторы `interceptors` в Spring?](sm.md#Что-такие-интерцепторы-interceptors-в-Spring)
+ [Нужно ли находиться на classpath файлу `spring-mvc.jar` или он уже присутствует в составе `spring-core`?](sm.md#Нужно-ли-находиться-на-classpath-файлу-spring-mvc.jar-или-он-уже-присутствует-в-составе-spring-core)
...
+ [Как выполняется валидация данных формы в фреймворке `Spring Web MVC`?](sm.md#Как-выполняется-алидация-данных-формы-в-фреймворке-Spring-Web-MVC)
...
+ [Как в Spring MVC поддерживаются  `i18n` международная адаптация и локализация?](sm.md#Как-в-Spring-MVC-поддерживаются-i18n-международная-адаптация-и-локализация?)
+ [Что вы понимаете под `MultipartResolver`?](sm.md#Что-вы-понимаете-под-MultipartResolver)
+ [Как можно использовать источник данных `Tomcat JNDI` Tomcat JNDI DataSource в приложениях Spring?](sm.md#Как-можно-использовать-источник-данных-Tomcat-JNDI-Tomcat-JNDI-DataSource-в-приложениях-Spring)
...
## Git
+ [Что такое система контроля версий и для чего она используется?](git.md#Что-такое-система-контроля-версий-и-для-чего-она-используется?)
+ [Чем отличается команда `git pull` от `git fetch`?](git.md#Чем-отличается-команда-git-pull-от-git-fetch?)
+ [Что такое конфликт слияния `merge conflict` и как его разрешить?](git.md#Что-такое-конфликт-слияния-merge-conflict-и-как-его-разрешить?)
+ [Как создать новую ветку в Git?](git.md#Как-создать-новую-ветку-в-Git)
+ [Что такое `Git stash` и как им пользоваться?](git.md#Что-такое-Git-stash-и-как-им-пользоваться)
+ [Как `отменить последний` коммит в Git?](git.md#Как-отменить-последний-коммит-в-Git?)
+ [Как `объединить ветку` с основной веткой master в Git?](git.md#Как-объединить-ветку-с-основной-веткой-master-в-Git)
+ [Как создать и применить `патч` в Git?](git.md#Как-создать-и-применить-патч-в-Git)
+ [Как `перенести коммиты` из одной ветки в другую в Git?](git.md#Как-перенести-коммиты-из-одной-ветки-в-другую-в-Git)
+ [Как `удалить` ветку в Git?](git.md#Как-удалить-ветку-в-Git)
+ [Как `создать` новую ветку?](git.md#Как-создать-новую-ветку)
+ [В чем разница между `git clone` и `git pull`?](git.md#В-чем-разница-между-git-clone-и-git-pull)
+ [Как `отменить` последний коммит?](git.md#Как-отменить-последний-коммит)
+ [Как проверить `различия` между файлами в Git?](git.md#Как-проверить-различия-между-файлами-в-Git)
+ [Как переместить файлы `под индекс` в Git?](git.md#Как-переместить-файлы-под-индекс-в-Git)
+ [Как `переключиться` на другую ветку в Git?](git.md#Как-переключиться-на-другую-ветку-в-Git)
+ [Как `скачать` изменения из удаленного репозитория `без объединения` с текущей веткой?](git.md#Как-скачать-изменения-из-удаленного-репозитория-без-объединения-с-текущей-веткой)
+ [Как отменить изменения в репозитории, если я не хочу вносить изменения в конкретный файл, а отменить все изменения в репозитории?](git.md#Как-отменить-изменения-в-репозитории,-если-я-не-хочу-вносить-изменения-в-онкретный-файл,-а-отменить-все-изменения-в-репозитории)
+ [Что делать, если я хочу `изменить сообщение` последнего коммита?](git.md#Что-делать,-если-я-хочу-изменить-сообщение-последнего-коммита)
+ [Как `отменить` определенный `коммит` в репозитории?](git.md#Как-отменить-определенный-коммит-в-репозитории)
+ [Что делает команда `git cherry pick` хэш_коммита?](git.md#Что-делает-команда-git-cherry-pick-хэш_коммита)
+ [Как проверить состояние файлов в репозитории с помощью команды `git status`?](git.md#Как-проверить-состояние-файлов-в-репозитории-с-помощью-команды-git-status)
+ [Что показывает команда `git log`?](git.md#Что-показывает-команда-git-og)
+ [Что происходит при выполнении команды `git pull`?](git.md#Что-происходит-при-выполнении-команды-git-pull)
+ [Что делает команда `git clean -xdf`?](git.md#Что-делает-команда-git-clean--xdf)
+ [Как выполнить интерактивный режим ребейза](git.md#Как-выполнить-интерактивный-режим-ребейза)
+ [Как отменить изменения, которые были проиндексированы, с помощью команды `git reset .`?](git.md#Как-отменить-изменения,-которые-были-проиндексированы,-с-помощью-команды-git-reset-.)

[к оглавлению](#вопросы-для-собеседования-на-java-developer)

## Build Tool
+ [Что такое `Build Tool` и зачем его использовать?](bt.md#Что-такое-Build-Tool-и-зачем-его-использовать)
+ [В чем разница между `Apache Maven` и `Gradle`?](bt.md#В-чем-разница-между-Apache-Maven-и-Gradle)
+ [Какие преимущества имеет `Gradle` перед `Maven` и `Ant`?](bt.md#Какие-преимущества-имеет-Gradle-перед-Maven-и-Ant)
+ [Что такое `Maven` и какие его основные преимущества?](bt.md#Что-такое-Maven-и-какие-его-основные-преимущества)
+ [Как устроен `pom` файл и какие его основные элементы?](bt.md#Как-устроен-pom-файл-и-какие-его-основные-элементы)
+ [Какие команды `Maven` вы знаете и как ими пользоваться?](bt.md#Какие-команды-Maven-вы-знаете-и-как-ими-пользоваться)
+ [Что такое dependency management в `Maven` и как он работает?](bt.md#Что-такое-dependency-management-в-Maven-и-как-он-работает)
+ [Что такое `Gradle` и какие его основные преимущества перед `Maven`?](bt.md#Что-такое-Gradle-и-какие-его-основные-преимущества-перед-Maven)
+ [Как устроен `build.gradle` файл и какие его основные элементы?](bt.md#Как-устроен-build.gradle-файл-и-какие-его-основные-элементы)
+ [Какие команды `Gradle` вы знаете и как ими пользоваться?](bt.md#Какие-команды-Gradle-вы-знаете-и-как-ими-пользоваться)
+ [Как установить и запустить `Gradle`?](bt.md#Как-установить-и-запустить-Gradle)
+ [Как работать с зависимостями в `Gradle`?](bt.md#Как-работать-с-зависимостями-в-Gradle?)

[к оглавлению](#вопросы-для-собеседования-на-java-developer)

## SPRING
- [Cобеседование по Java. Разбор вопросов и ответов.](jsf.md#cобеседование-по-java-разбор-вопросов-и-ответов)
- [SPRING](jsf.md#spring)
- [Что такое Spring](jsf.md#что-такое-spring)
 - [Особенности и преимущества Spring Framework?](jsf.md#особенности-и-преимущества-spring-framework)
- [Spring контейнеры](jsf.md#spring-контейнеры)
- [Жизненный цикл Context](jsf.md#жизненный-цикл-context)
- [Как завершить работу контекста](jsf.md#как-завершить-работу-контекста)
- [Bean](jsf.md#bean)
- [Жизненный цикл бинов](jsf.md#жизненный-цикл-бинов)
- [Как настроить класс как Spring Bean](jsf.md#как-настроить-класс-как-spring-bean)
- [Статический Bean](jsf.md#статический-bean)
- [Inversion of Control](jsf.md#inversion-of-control)
- [Dependency Injection (DI)](jsf.md#dependency-injection-di)
- [Как реализуется DI в Spring Framework?](jsf.md#как-реализуется-di-в-spring-framework)
- [Связывание и @Autowired](jsf.md#связывание-и-autowired)
- [MVC](jsf.md#mvc)
- [Шаблон проектирования Front Controller](jsf.md#шаблон-проектирования-front-controller)
- [В чем разница между Filters, Listeners и Interceptors?](jsf.md#в-чем-разница-между-filters-listeners-и-interceptors)
- [Связывание форм](jsf.md#связывание-форм)
- [Исключения в Spring MVC](jsf.md#исключения-в-spring-mvc)
- [Локализация в приложениях Spring MVC](jsf.md#локализация-в-приложениях-spring-mvc)
- [Spring Interceptor](jsf.md#spring-interceptor)
- [CommandLineRunner и ApplicationRunner](jsf.md#commandlinerunner-и-applicationrunner)
- [Реактивное программирование](jsf.md#реактивное-программирование)
- [Паттерны в Spring Framework](jsf.md#паттерны-в-spring-framework)
- [AOP и составные части](jsf.md#aop-и-составные-части)
- [Spring AOP vs ASPECTJ](jsf.md#spring-aop-vs-aspectj)
- [Некоторые частые аннотации Spring](jsf.md#некоторые-частые-аннотации-spring)
- [Различия @Component, @Service, @Repository, @Controller](jsf.md#различия-component-service-repository-controller)
- [Различия @Controller и @RestController](jsf.md#различия-controller-и-restcontroller)
- [@Qualifier and @Primary](jsf.md#qualifier-and-primary)
- [@Profile](jsf.md#profile)
- [@LookUp](jsf.md#lookup)
- [@Target и @Retention](jsf.md#target-и-retention)
- [@Resource](jsf.md#resource)
- [@Inject](jsf.md#inject)
- [@Autowired vs @Resource vs @Inject](jsf.md#autowired-vs-resource-vs-inject)
- [@Conditional](jsf.md#conditional)
- [Как управлять транзакциями в Spring](jsf.md#как-управлять-транзакциями-в-spring)
- [новая инфа](jsf.md#новая-инфа)
- [старая инфа](jsf.md#старая-инфа)
- [Как Spring работает с DAO](jsf.md#как-spring-работает-с-dao)
- [Model vs ModelMap vs ModelAndView](jsf.md#model-vs-modelmap-vs-modelandview)
- [В чем разница между model.put() и model.addAttribute()?](jsf.md#в-чем-разница-между-modelput-и-modeladdattribute)
- [PreparedStatementCreator](jsf.md#preparedstatementcreator)
- [SOAP vs REST](jsf.md#soap-vs-rest)
- [Spring Data](jsf.md#spring-data)
- [Конфигурация Spring Data](jsf.md#конфигурация-spring-data)
- [Spring Security](jsf.md#spring-security)
- [Spring Boot](jsf.md#spring-boot)
- [Starter packs](jsf.md#starter-packs)
- [Как внедрить java.util.Properties в Spring Bean](jsf.md#как-внедрить-javautilproperties-в-spring-bean)
- [Что нового в Spring 5](jsf.md#что-нового-в-spring-5)
- [RestTemplate и JDBCTemplate](jsf.md#resttemplate-и-jdbctemplate)
- [Socket](jsf.md#socket)

[к оглавлению](#вопросы-для-собеседования-на-java-developer)

## Spring Security
+ [Что такое делегирующий `прокси-фильтр`в Spring Security?](ssf.md#Что-такое-делегирующий-прокси-фильтр-в-Spring-Security)
+ [Какие существуют ограничения на использование `делегирующего фильтрующего прокси` в системе безопасности Spring?](ssf.md#Какие-существуют-ограничения-на-использование-делегирующего-фильтрующего-прокси-в-системе-безопасности-Spring)
+ [Будут ли методы `жизненного цикла Fitler`, такие как init и destroy, делегироваться целевому Bean с помощью DelegatingFilterProxy?](ssf.md#Будут-ли-методы-жизненного-цикла-Fitler,-такие-как-init-и-destroy,-делегироваться-целевому-Bean-с-помощью-DelegatingFilterProxy)
+ [Кто управляет `жизненным циклом Bean` фильтра в Spring?](ssf.md#Кто-управляет-жизненным-циклом-Bean-фильтра-в-Spring)
+ [Что представляет собой `цепочка фильтров` безопасности в `Spring Security`?](ssf.md#Что-представляет-собой-цепочка-фильтров-безопасности-в-Spring-Security)
+ [Какие предопределенные фильтры используются в `Spring Security`?](ssf.md#Какие-предопределенные-фильтры-используются-в-Spring-Security)
+ [Можно ли добавить пользовательские фильтры в цепочку фильтров `Spring security`?](ssf.md#Можно-ли-добавить-пользовательские-фильтры-в-цепочку-фильтров-Spring-security)
+ [Как реализовать пользовательский `фильтр в Spring Security`?](ssf.md#Как-реализовать-пользовательский-фильтр-в-Spring-Security)
+ [Как добавить пользовательский фильтр в цепочку фильтров Spring Security?](ssf.md#Как-добавить-пользовательский-фильтр-в-цепочку-фильтров-Spring-Security)
+ [Является ли безопасность сквозной задачей?](ssf.md#Является-ли-безопасность-сквозной-задачей)
+ [Для чего используются @ и # в `Spring Expression Language`?](ssf.md#Для-чего-используются-@-и-#-в-Spring-Expression-Language)
+ [Для каких аннотаций безопасности разрешено использовать `SpEL`?](ssf.md#Для-каких-аннотаций-безопасности-разрешено-использовать-SpEL)
+ [Что такое `контекст безопасности`?](ssf.md#Что-такое-контекст-безопасности)
+ [Что такое `аутентификация` и `авторизация`?](ssf.md#Что-такое-аутентификация-и-авторизация)
+ [Что такое `принцип` в системе безопасности Spring?](ssf.md#Что-такое-принцип-в-системе-безопасности-Spring)
+ [Зачем нужен `intercept-url`?](ssf.md#Зачем-нужен-intercept-url)
+ [Зачем нужна защита метода?](ssf.md#Зачем-нужна-защита-метода)
+ [В каком порядке нужно писать несколько intercept-url'ов?](ssf.md#В-каком-порядке-нужно-писать-несколько-intercept-url'ов)
+ [Что делают `@Secured` и `@RolesAllowed`?](ssf.md#Что-делают-@Secured-и-@RolesAllowed)
+ [Поддерживает ли Spring Security хеширование паролей?](ssf.md#Поддерживает-ли-Spring-Security-хеширование-паролей)
+ [Что такое `PasswordEncoder`?](ssf.md#то-такое-PasswordEncoder)
+ [Каковы некоторые реализации `PasswordEncoder` в Spring Security?](ssf.md#Каковы-некоторые-реализации-PasswordEncoder-в-Spring-Security)
